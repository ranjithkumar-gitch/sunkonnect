...
class _ForgotpasswardState extends State<Forgotpassward> {
  ...
  VerifyEmailresponseModel? _verifyEmailresponseModel;
              ...
              if (_formKey.currentState!.validate()) {
                ...
                apiService.getSecretCode().then((value) {
                  if (value.status == 203) {
                    ...
                    requestModelId.userId =
                        emailController.text.toString().trim();
                    verifyEmail(emailController.text.toString().trim());
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) => OTPScreen(
                      VerificationCode: value.data!.token.toString(),
                    )));
      } else {
        setState(() {
          isApiCallProcess = false;
  ...
  verifyEmail(String email) {
    ...
    apiService.verifyEmail(email).then((value) {
      if (value.status == 203) {
        setState(() {
          isApiCallProcess = false;
        });
        ...
        showToast("Verification email  Successfully");
        // print("login url is working perfect uday");
        //loginId, userId, name, roleCode, roleDescription, status, accessToken, refreshToken, userObjId//
        // setState(() {
        //   // SharedPrefServices.setuserId(requestModelId.userId);
